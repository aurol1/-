clear,clc,close all;
imgPath = 'D:\pythonMat\archive\CUB_200_2011\CUB_200_2011\images\001.Black_footed_Albatross\Black_Footed_Albatross_0001_796111.jpg';
bgPath = 'D:\pythonMat\archive\segmentations\segmentations\001.Black_footed_Albatross\Black_Footed_Albatross_0001_796111.png';
img = imread(imgPath);
bg = imread(bgPath);
self = lapulasi(img);
img = rgb2gray(img);
ku = fspecial('laplacian');
J=imfilter(img,ku,'replicate');%图像滤波，kreplicateu边界选项
K=imbinarize(J,30/255);%变为二维图像
figure,
% subplot(3,1,1),
imshow(img),
title("原图");
